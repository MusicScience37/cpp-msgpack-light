...
TEST_CASE(
    "msgpack_light::type_support::serialization_traits<std::deque<int>>") {
    ...
    SECTION("serialize") {
        ...
        std::tie(value, expected_binary) = GENERATE(
            table<std::deque<int>, binary>({{std::deque<int>(), binary("90")},
                {std::deque<int>{0x2A}, binary("912A")},
                {std::deque<int>{0x2A, 0x3B}, binary("922A3B")},
                {std::deque<int>{0x2A, 0x3B, 0x4C}, binary("932A3B4C")}}));
